...
class TicTacToeState : public State {
 public:
  ...
  void forward(Action actionIndex) {
    assert(actionIndex != InvalidAction);
    std::vector<_Action> theLegalActions = getLegalActions();
    _Action a = theLegalActions.at(actionIndex);
    int ticTacToeMove = a.GetX();
    assert(ticTacToeMove >= 0 && ticTacToeMove <= 8);
    // std::cout << "previously, the board looked like " << std::endl;
    // printState();
    board[ticTacToeMove] = currentPlayer;

    _legalActions = getLegalActions();  // this semes to have made no difference
    currentPlayer = -currentPlayer;
    moveIdx += 1;
    // std::cout << "the action index was " << actionIndex << std::endl;
    // std::cout << "the legal moves were " << theLegalActions << std::endl;
    // std::cout << "so, we chose move " << a << std::endl;
    // std::cout << "which had x component " << a.GetX();
    // std::cout << "now, the board looks like " << std::endl;
    // printState();
  }
